...
<?php
	...
	// $products = $mysqli->query("SELECT p.id,p.name as name ,p.id_seller, u.name as sellerName, t.main_type1, t.main_type2,t.main_type3, p.price, p.description,p.sale, l.list_name as listC, pAd.advert_name as AdvertName, pAd.decreat_price,pAd.decreat_percent, GROUP_CONCAT(i.link) AS images FROM product p LEFT JOIN images i ON p.id = i.id_product inner join producttype t ON t.id_product = p.id inner join productlistc l ON p.id_list = l.id inner join users u ON p.id_seller = u.id inner join product_advert pAd ON p.id = pAd.id_product where p.id_seller = '$id_seller' group by p.id LIMIT 0,6");
