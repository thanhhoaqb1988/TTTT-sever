...
<?php
	...
	$products = $mysqli->query("SELECT p.id,p.name as name ,p.id_seller, t.main_type1, t.main_type2,t.main_type3, p.price, p.description,p.sale, l.list_name, l.id as idList, GROUP_CONCAT(i.link) AS images FROM product p LEFT JOIN images i ON p.id = i.id_product inner join producttype t ON t.id_product = p.id inner join productlista l ON p.id_list = l.id where p.id_seller = '$id_seller' group by p.id LIMIT 0,6");
