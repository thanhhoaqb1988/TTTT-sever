...
<?php
    ...
    $id_seller= $obj['sellerId'];
     //$id_seller= '68';
	$products = $mysqli->query("SELECT p.*, u.name as sellerName,u.phone,s.shopAdress, t.main_type1, t.main_type2,t.main_type3, l.list_name as listC, a.*,GROUP_CONCAT(i.link) AS images FROM product p LEFT JOIN images i ON p.id = i.id_product inner join producttype t ON t.id_product = p.id inner join productlistc l ON p.id_list = l.id inner join users u ON p.id_seller = u.id inner join product_advert a ON p.sale=a.id inner join shopinfor s ON p.id_seller = s.sellerId  where p.id_seller = '$id_seller' group by p.id LIMIT 0,6");
